...
const DEFAULT_SYSTEM_PROMPT = `You are a legal compliance analyzer. Identify legal requirements and risks.
...
export async function POST(request: NextRequest) {
  try {
    const body: LegalComplianceRequest = await request.json();
    ...
    console.log('⚖️ Starting Legal Compliance analysis...');
        ...
        if (claudeMessage.type === "assistant" && claudeMessage.content) {
          for (const block of claudeMessage.content) {
            if (block.type === "text" && block.text) {
              console.log(`Processing legal compliance... ${block.text.substring(0, 100)}...`);
            }
          }
...
export async function GET() {
  return NextResponse.json({
    status: 'Legal Compliance Analysis API is running',
