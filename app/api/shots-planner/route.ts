...
const DEFAULT_SYSTEM_PROMPT = `You are a shots planner. Create detailed shot lists and coverage plans.
...
export async function POST(request: NextRequest) {
  try {
    const body: ShotsPlannerRequest = await request.json();
    ...
    console.log('🎯 Starting Shots Planner analysis...');
        ...
        if (claudeMessage.type === "assistant" && claudeMessage.content) {
          for (const block of claudeMessage.content) {
            if (block.type === "text" && block.text) {
              console.log(`Processing shots... ${block.text.substring(0, 100)}...`);
            }
          }
...
export async function GET() {
  return NextResponse.json({
    status: 'Shots Planner API is running',
