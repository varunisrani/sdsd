...
interface StoryboardDesignerRequest {
  prompt: string;
  apiKey?: string;
  maxTurns?: number;
  systemPrompt?: string;
...
const DEFAULT_SYSTEM_PROMPT = `You are a storyboard designer. Create visual planning reports. 
...
export async function POST(request: NextRequest) {
  try {
    const body: StoryboardDesignerRequest = await request.json();
    ...
    console.log('🎬 Starting Storyboard Designer analysis...');
...
export async function GET() {
  return NextResponse.json({
    status: 'Storyboard Designer API is running',
