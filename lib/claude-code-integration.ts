// Client-side integration for Claude Code API
...
export const DEFAULT_CLAUDE_CODE_CONFIG: ClaudeCodeConfig = {
  maxTurns: 25,
  systemPrompt: `You are a script parser. Analyze scripts and create detailed breakdown reports. 
...
- Process it chunk by chunk YOURSELF
- Use Write tool to create the report
- IGNORE file size - work through it systematically yourself
...
export class ClaudeCodeAnalysisService {
  ...
  async analyzeScript(prompt: string, additionalContext?: string): Promise<ClaudeCodeAnalysisResult> {
    const startTime = Date.now();
    
    try {
      console.log('🤖 Starting Claude Code analysis via API...');
      
      if (!this.config.apiKey?.trim()) {
        throw new Error('API key is required for Claude Code analysis');
      }
      ...
      console.log(`✅ Claude Code analysis completed in ${executionTime}ms`);
...
// Utility functions for managing Claude Code data
export class ClaudeCodeDataUtils {
  ...
  static getAnalysisStats(result: ClaudeCodeAnalysisResult) {
